...
- **Caps and spend (policy)**  
  All caps (`capPerTxMinor`, `capPerSessionMinor`, `capPerDayMinor`) and spend totals are in **fiat minor** (lot currency, e.g. USD cents). Entry grant stores these; exit evaluation compares price and spend in the **same unit** (lot currency, fiat minor). The decision record stores `quote_minor` + `quote_currency` (same fiat minor). No mixing of fiat and stablecoin in cap checks.
